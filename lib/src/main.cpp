...
int main() {
    ...
    // 输出到文件
    logger.set_log_file("mdbsrv.log");
    // 启用/禁用控制台输出
    ...
    db->addTableFromFile("table.json");
    ...
    users->importRowsFromFile("data.json");
    ...
    users1->showRows();
    // Save to JSON file
    users1->exportToFile("users-1.json");
    users->exportToFile("users.json");

    std::cout << "Table saved to users.json" << std::endl;
