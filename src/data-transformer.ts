...
export class DataTransformer {
	...
	private transformRows(mappings: Mapping[], dataSources: { [key: string]: Row[] }): Row[] {
		// 确定所有源表
		const uniqueSources = Array.from(new Set(mappings.map((mapping) => mapping.source?.name).filter(Boolean)));
	
		// 检查数据源是否存在
		uniqueSources.forEach((sourceName) => {
			if (!dataSources[sourceName]) {
				throw new Error(`Source table "${sourceName}" not found in dataSources.`);
			}
		});
		...
		return primarySourceRows.map((primaryRow) => 
			mappings.reduce((transformedRow, mapping) => {
				...
				const matchingRow = dataSources[source?.name]?.find(row => row[source.field] === primaryRow[source.field]);
				let value = matchingRow?.[source.field];
				
				if (transform) {
					value = typeof transform === "string" 
    ...
    async saveTables(destConfig: DataDestinationConfig) {
		...
		for (const mappingConfig of destConfig.inmemory) {
            const destination = destConfig.destinations.find(dest => dest.name === mappingConfig.destination);
            if (!destination) {
    ...
    private handlers: { [key in TransformType]: (config: TransformDock, rows?: Row[]) => Promise<Row[] | void> } = {
        [TransformType.EXCEL]: async (config, rows) => rows ? this.saveToExcel(config.path!, rows, config.sheetName) : this.loadFromExcel(config.path!),
        [TransformType.CSV]: async (config, rows) => rows ? this.saveToJSON(config.path!, rows) : this.loadFromCsv(config.path!),
        [TransformType.JSON]: async (config, rows) => rows ? null : this.loadFromJSON(config.path!),
        [TransformType.DATABASE]: async (config, rows) => rows ? this.saveToDatabase(config.query!, rows) : this.loadFromDatabase(config.query!),
        [TransformType.API]: async (config, rows) => rows ? this.sendToApi(config.apiUrl!, rows) : this.loadFromApi(config.apiUrl!),
    };

    async processTransformDock(config: TransformDock, rows?: Row[]): Promise<Row[] | void> {
        ...
        try {
            ...
            throw new Error(`Error processing transform dock for type ${config.type}: ${error.message}`);
        }
    }
}
