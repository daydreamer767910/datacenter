...
export class MemTable implements Table {
	...
	public validateRow(row: Row): boolean {
		for (const column of this.columns) {
			...
			if (value !== null && value !== undefined && !this.isValidType(value, column.type)) {
	...
	private validatePrimaryKey(row: Row): boolean {
		...
		if (primaryKeyColumn) {
			...
			if (primaryKeyValue !== undefined && primaryKeyValue !== null) {
				// 检查主键值是否已经存在
				const existingRow = this.rows.find(existingRow => existingRow[primaryKeyColumn.name] === primaryKeyValue);
				if (existingRow) {
					throw new Error(`Primary key value "${primaryKeyValue}" already exists.`);
				}
			}
		}
		return true;
